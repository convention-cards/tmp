...
const initialValues: FormType = {
  attitude: CardingType.None,
  attitudeOther: "",
  count: CardingType.None,
  countOther: "",
  suitPref: CardingType.None,
  suitPrefOther: "",
  smith: CardingType.None,
  smithOther: "",
  discards: DiscardType.None,
  discardsOther: "",

  notes: "",
};

export function DefineCardingDialog({ systemId, onClose }: Props) {
  const ctx = api.useContext();
  const addCarding = api.carding.add.useMutation({
    onSuccess: () => {
      ctx.system.get.invalidate();
    },
  });
  const onSubmit = async (v: FormType) => {
    await addCarding.mutateAsync({ ...v, systemId });
